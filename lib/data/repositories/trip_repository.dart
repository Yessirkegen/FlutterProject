...
class TripRepository {
  ...
  Future<bool> syncTrips(List<Map<String, dynamic>> localTrips) async {
    ...
    try {
      ...
      for (final trip in localTrips) {
        ...
        batch.set(docRef, normalizedTrip, SetOptions(merge: true));
      }
  ...
  // Helper method to normalize trip field names for Firebase
  Map<String, dynamic> _normalizeTrip(Map<String, dynamic> trip) {
    final normalizedTrip = Map<String, dynamic>.from(trip);
    
    // Convert any 'start_date' to 'startDate' for consistency
    if (normalizedTrip.containsKey('start_date') && !normalizedTrip.containsKey(AppConstants.tripStartDate)) {
      normalizedTrip[AppConstants.tripStartDate] = normalizedTrip.remove('start_date');
    }
    
    // Convert any 'end_date' to 'endDate' for consistency
    if (normalizedTrip.containsKey('end_date') && !normalizedTrip.containsKey(AppConstants.tripEndDate)) {
      normalizedTrip[AppConstants.tripEndDate] = normalizedTrip.remove('end_date');
    }
